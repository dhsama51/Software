// M2 experiment 1: find column candidates.
// Fixed anchor: GPIO[9] is held LOW, which was observed as R1.
// Each JOY_PRESS advances probe_idx and drives one GPIO HIGH.
// If the LED matrix shows (x, 1), the probed GPIO is column Cx.
module top (
    input  wire CLOCK_50,
    ...
    localparam [3:0] ANCHOR_ROW_GPIO = 4'd9;
    ...
    generate
        for (i = 0; i < 16; i = i + 1) begin : gpio_drive
            if (i == ANCHOR_ROW_GPIO) begin : anchor_pin
                assign GPIO[i] = 1'b0;
            end else begin : probe_pin
                assign GPIO[i] = (probe_idx == i[3:0]) ? 1'b1 : 1'bz;
            end
        end
    endgenerate
